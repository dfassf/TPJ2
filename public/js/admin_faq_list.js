function init(){
    ...
    addBtn.addEventListener('click',function(){
        window.location.href ='http://localhost:3000/admin/faq/add'
    })
    ...
    srcBtn.addEventListener('click',function(){
        if(srcStartDate.value > srcEndDate.value && srcEndDate.value !== ''){
            ...
            console.log('검색출력');
            window.location.href=`http://localhost:3000/admin/faq/search?title=${title.value}&category=${srcCategory.value}&show=${srcShow.value}&start_date=${srcStartDate.value}&end_date=${srcEndDate.value}`
        }
    })
